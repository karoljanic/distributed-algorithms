...
int main(int argc, char** argv) {
    ...
    Simulator<BfsState, BfsMessage> simulator(graph, is_sync, std::stoul(min_delay_str), std::stoul(max_delay_str));

    simulator.set_handlers(
        [](Simulator<BfsState, BfsMessage>& sim, size_t node_id, BfsState& state) {
            state.visited = true;
            for (size_t neighbor : sim.get_graph().neighbors(node_id)) {
                sim.send_message(node_id, neighbor, BfsMessage{0});
            }
        },
        [](Simulator<BfsState, BfsMessage>& sim, size_t node_id, BfsState& state, size_t sender_id, const BfsMessage& message) {
            if (!state.visited) {
                state.visited = true;
                ...
                for (size_t neighbor : sim.get_graph().neighbors(node_id)) {
                    sim.send_message(node_id, neighbor, BfsMessage{state.level});
                }
            }
        }
    );
    ...
    for(size_t node_id : graph.nodes()) {
        const BfsState& state = simulator.get_node_state(node_id);
